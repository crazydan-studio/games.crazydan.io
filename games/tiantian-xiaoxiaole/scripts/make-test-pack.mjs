...
// 用法：在游戏子模块目录下执行 node scripts/make-test-pack.mjs [输出路径]
...
const out = process.argv[2] || join(GAME_ROOT, '.tmp', 'test-pack.zip')
const zip = new JSZip()
...
const items = faces.map((f, i) => {
  ...
  zip.file(file, readFileSync(join(GAME_ROOT, 'public/expressions/images', f)))
